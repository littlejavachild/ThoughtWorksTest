...
public class CommandProvider {
	...
	private boolean isTranslate(String str) {
		...
		if(!Pattern.matches("\\w+", tokens[0])) return false;
		// middle token should be "is"
		if(!tokens[1].equals("is")) return false;
		// last token should be a roman number of size 1
		if(!"IVXLCDM".contains(tokens[2]) && tokens[2].length() == 1) return false;
		return true;
	}
	//------------------------------------------------------------------------------
	private boolean isGetCredits(String str){
		str = str.trim();
		String[] tokens = str.split("\\s");
		if(tokens[0].equals("how") 
		   && tokens[1].equals("many") 
		   && tokens[2].equals("Credits")
		   && tokens[3].equals("is")
		   && dict.getCommodities().contains(tokens[tokens.length-2])
		   && areAlienWords(4, tokens.length-3, tokens)
		   && tokens[tokens.length-1].equals("?")) return true;
		return false;
	}
	//------------------------------------------------------------------------------
	private boolean isGetValue(String str){
		...
		String[] tokens = str.split("\\s");
		if(tokens[0].equals("how")
		   && tokens[1].equals("much")
		   && tokens[2].equals("is")
		   && areAlienWords(3,tokens.length-2,tokens)
		   && tokens[tokens.length-1].equals("?")) return true;
		return false;
	}
	//------------------------------------------------------------------------------
	private boolean isAddCredits(String str){
		str = str.trim();
		String[] tokens = str.split("\\s");
		if(tokens[tokens.length-1].equals("Credits")
		   && Pattern.matches("\\d+", tokens[tokens.length-2])
		   && tokens[tokens.length-3].equals("is")
		   && !dict.getTranslatedAlienWords().contains(tokens[tokens.length-4])
		   && areAlienWords(0, tokens.length-5, tokens)) return true;
		return false;
	}
	//------------------------------------------------------------------------------
	private boolean areAlienWords(int start,int end,String[] words){
